...
export class Terminal extends EventEmitter
  implements ITerminal, IDisposable, IInputHandlingTerminal {
  ...
  private _initGlobal(): void {
    ...
    this.register(
      addDisposableDomListener(this.element, 'copy', (event: ClipboardEvent) => {
    ...
    if (Browser.isFirefox) {
      ...
      this.register(
        addDisposableDomListener(this.element, 'mousedown', (event: MouseEvent) => {
          if (event.button === 2) {
            rightClickHandler(
              event,
              this.textarea,
      ...
      this.register(
        addDisposableDomListener(this.element, 'contextmenu', (event: MouseEvent) => {
          rightClickHandler(
            event,
            this.textarea,
            this.selectionManager,
    ...
    if (Browser.isLinux) {
      ...
      this.register(
        addDisposableDomListener(this.element, 'auxclick', (event: MouseEvent) => {
          if (event.button === 1) {
  ...
  private _bindKeys(): void {
    ...
    this.register(
      addDisposableDomListener(this.textarea, 'compositionstart', () =>
        this._compositionHelper.compositionstart()
      )
    )
    this.register(
      addDisposableDomListener(this.textarea, 'compositionupdate', (e: CompositionEvent) =>
        this._compositionHelper.compositionupdate(e)
      )
    )
    this.register(
      addDisposableDomListener(this.textarea, 'compositionend', () =>
        this._compositionHelper.compositionend()
      )
    )
    this.register(
      this.addDisposableListener('refresh', () =>
        this._compositionHelper.updateCompositionElements()
      )
    )
    this.register(
      this.addDisposableListener('refresh', data => this._queueLinkification(data.start, data.end))
    )
  }
  ...
  public open(parent: HTMLElement): void {
    ...
    this.register(
      addDisposableDomListener(this.textarea, 'focus', (ev: KeyboardEvent) =>
        this._onTextAreaFocus(ev)
      )
    )
    ...
    this.register(
      this.addDisposableListener('resize', () => this.renderer.onResize(this.cols, this.rows))
    )
    ...
    this.register(
      this.addDisposableListener('dprchange', () =>
        this.renderer.onWindowResize(window.devicePixelRatio)
      )
    )
    ...
    this.register(
      this.charMeasure.addDisposableListener('charsizechanged', () =>
        this.renderer.onCharSizeChanged()
      )
    )
    this.register(
      this.renderer.addDisposableListener('resize', dimensions => this.viewport.syncScrollArea())
    )

    this.selectionManager = new SelectionManager(this, this.charMeasure)
    this.register(
      addDisposableDomListener(this.element, 'mousedown', (e: MouseEvent) =>
        this.selectionManager.onMouseDown(e)
      )
    )
    this.register(
      this.selectionManager.addDisposableListener('refresh', data =>
        this.renderer.onSelectionChanged(data.start, data.end, data.columnSelectMode)
      )
    )
    this.register(
      this.selectionManager.addDisposableListener('newselection', text => {
    ...
    this.register(
      this.addDisposableListener('scroll', () => {
        this.viewport.syncScrollArea()
        this.selectionManager.refresh()
      })
    )
    this.register(
      addDisposableDomListener(this._viewportElement, 'scroll', () =>
        this.selectionManager.refresh()
      )
    )
  ...
  public bindMouse(): void {
    ...
    function sendMove(ev: MouseEvent): void {
      let button = pressed
      const pos = self.mouseHelper.getRawByteCoords(
        ev,
        self.screenElement,
        self.charMeasure,
        self.cols,
    ...
    function getButton(ev: MouseEvent): number {
      ...
      switch ((<any>ev).overrideType || ev.type) {
        ...
        case 'DOMMouseScroll':
          ...
          break
        case 'wheel':
          button = (<WheelEvent>ev).deltaY < 0 ? 64 : 65
          break
      }
    ...
    this.register(
      addDisposableDomListener(el, 'mousedown', (ev: MouseEvent) => {
    ...
    this.register(
      addDisposableDomListener(el, 'wheel', (ev: WheelEvent) => {
        if (!this.mouseEvents) {
          ...
          if (!this.buffer.hasScrollback) {
            ...
            const sequence =
              C0.ESC + (this.applicationCursor ? 'O' : '[') + (ev.deltaY < 0 ? 'A' : 'B')
            let data = ''
    ...
    this.register(
      addDisposableDomListener(el, 'touchstart', (ev: TouchEvent) => {
        if (this.mouseEvents) return
    ...
    this.register(
      addDisposableDomListener(el, 'touchmove', (ev: TouchEvent) => {
        if (this.mouseEvents) return
  ...
  public scroll(isWrapped: boolean = false): void {
    ...
    if (useRecycling) {
      ...
      if (
        ...
        newLine.get(0)[CHAR_DATA_ATTR_INDEX] !== this.eraseAttr()
      ) {
