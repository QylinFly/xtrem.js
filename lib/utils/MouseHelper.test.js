...
describe('MouseHelper.getCoords', function () {
    ...
    describe('when charMeasure is not initialized', function () {
        it('should return null', function () {
            charMeasure = new TestUtils_test_1.MockCharMeasure();
            chai_1.assert.equal(mouseHelper.getCoords({ pageX: 0, pageY: 0 }, document.createElement('div'), charMeasure, 10, 10), null);
        });
    });
    describe('when pageX/pageY are not supported', function () {
        it('should return null', function () {
            chai_1.assert.equal(mouseHelper.getCoords({ pageX: undefined, pageY: undefined }, document.createElement('div'), charMeasure, 10, 10), null);
        });
    });
